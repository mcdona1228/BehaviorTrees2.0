...
public class SquishDoor : Task
{
    Rigidbody mDoor;

    public SquishDoor(Rigidbody someDoor)
    {
        mDoor = someDoor;
    }
    ...
    public override void run()
    {
        Debug.Log("Squished door");
        mDoor.AddForce(0, -.5f, 0, ForceMode.VelocityChange);
        succeeded = true;
        EventBus.TriggerEvent(TaskFinished);
    }
}
...
public class GiveUp : Task
{
    GameObject mEntity;

    public GiveUp(GameObject someEntity)
    {
        mEntity = someEntity;
    }
...
public class HulkRage : Task
{
    GameObject mEntity;

    public HulkRage(GameObject someEntity)
    {
        mEntity = someEntity;
    }
